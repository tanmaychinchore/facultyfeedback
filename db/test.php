...
if($sem_type=='Odd')
  $s2 = "SELECT course_code,class,sem,section_or_batch from courses_faculty where f_id='$f_id' and sem%2=1 and acad_year='$acad_year' and dept_id='$dept_id'";
else
  $s2 = "SELECT course_code,class,sem,section_or_batch from courses_faculty where f_id='$f_id' and sem%2=0 and acad_year='$acad_year' and dept_id='$dept_id'";
  $r2= $conn->query($s2);
  while($res2=$r2->fetch_assoc()):
    ...
    while($res3=$r3->fetch_assoc()){
      $cname=$res3['c_name'];
      include ('print_report_header.php');
    ...
    <table style="border-radius: 5px;"><tr>
      ...
      while($row=$result->fetch_assoc()):
        ...
        if($status==0){
          ...
          $check = "SELECT response FROM response_midsem where q_id='$q_id' and course_code='$c_id' and f_id='$f_id' and acad_year='$acad_year' and sem_type='$sem_type' and roll_no in(".implode(',',$roll_no).")";
          ...
          $check = "SELECT response FROM response_endsem where q_id='$q_id' and course_code='$c_id' and f_id='$f_id' and acad_year='$acad_year' and sem_type='$sem_type' and roll_no in(".implode(',',$roll_no).")";
          $res = $conn->query($check);   
          while($response=$res->fetch_assoc()){
            $options[(int)$response["response"]-1]++;
          }
    ...
    if($status==0){
     ...
     while($comment=$res->fetch_assoc()){
      if($comment["comment"]=="" or $comment["comment"] =="-" or $comment["comment"] =="." or $comment["comment"] ==".." or $comment["comment"] =="-" or $comment["comment"] =="--" or $comment["comment"] ==" " or $comment["comment"]=="NA" or $comment["comment"]=="na" or $comment["comment"] == "none")
        continue;
